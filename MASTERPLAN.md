...
### Phase 1 — Convert the login path (highest priority, everyone goes through this door) — CORRECTED FINDING 2026-07-23, effectively COMPLETE for real traffic
  1. [x] **Corrected finding (2026-07-23 night):** real production login on `app.marga.biz` does NOT run `netlify/functions/login.js`. Routing is `app.marga.biz` -> Cloudflare Tunnel -> `scripts/local-margabase-proxy.mjs` (port 9100), which special-cases `/api/login` to call its own `nativeEmployeeLogin()` function directly against Postgres via a raw `pg.Pool` connection (`app_meta.firestore_documents`, `tbl_employee`). This function has zero Firebase/Firestore calls of any kind. Verified live 2026-07-23 ~9:xx PM Manila: `POST https://app.marga.biz/api/login` with bad credentials returns `{"success":false,"message":"Invalid email or password"}` / HTTP 401 from the native handler, and both `com.marga.app-proxy` and `com.marga.margabase-firestore-api` launchd services are confirmed running.
  2. [x] This means today's incident (Phase 0) and its fix were entirely consistent with this: `nativeEmployeeLogin()` reads the same Postgres table the Phase 0 resync corrected. There was never a live Firebase call in the actual login path being exercised by staff.
  3. [ ] Remaining cleanup (low priority, not on the critical path, do NOT rush this at night before a staff login window): `netlify/functions/login.js` still contains full legacy Firebase/Firestore code (Google service-account OAuth token minting, hardcoded `firestore.googleapis.com` default). It is confirmed unreachable for `app.marga.biz` traffic (the proxy intercepts `/api/login` before `runLocalNetlifyFunction` would ever load this file) and is blocked immediately for `margaapp.netlify.app` traffic via the existing `isLegacyNetlifyHost()` check at the top of its handler, before any Firebase code runs. So it is dead code today, but it should still be rewritten to call the same Postgres path (or simply retired the way the two scheduled auto-sync functions were) as part of Phase 2/hygiene pass, purely so no Firebase-shaped code remains anywhere in the repo, per the owner's directive. Do this in a normal work session with time to verify, not late at night.
