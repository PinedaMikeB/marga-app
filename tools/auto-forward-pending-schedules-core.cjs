...
function normalizeLegacyDateTime(value) {
  ...
  if (
    ["", ZERO_DATETIME, "undefined", "undefined 00:00:00", "null", "null 00:00:00", "invalid date", "nan"].includes(compact)
  ) return "";
  return text;
}
...
function resolveBaseUrl() {
  return (
    readEnv("MARGABASE_DOCUMENTS_BASE_URL")
    || readEnv("MARGABASE_FIRESTORE_BASE_URL")
    || DEFAULT_BASE_URL
  );
}

function resolveApiKey() {
  return readEnv("MARGABASE_API_KEY") || DEFAULT_MARGABASE_API_KEY;
}
...
class FirestoreClient {
  constructor() {
    ...
    if (this.baseUrl.includes(LEGACY_FIRESTORE_HOST) && process.env.ALLOW_LEGACY_FIREBASE_WRITES !== "1") {
      throw new Error(
        "Blocked legacy Firebase backend. Set ALLOW_LEGACY_FIREBASE_WRITES=1 only for an explicitly approved rescue.",
      );
    }
  }
  ...
  async queryDateRange(collectionId, fieldPath, start, end, limit = QUERY_LIMIT) {
    return this.query({
      from: [{ collectionId }],
      where: {
        compositeFilter: {
  ...
  async queryDateRangeBatched(collectionId, fieldPath, startDateKey, endDateKey, batchDays = QUERY_BATCH_DAYS) {
    const rows = [];
    let cursor = startDateKey;
    while (cursor <= endDateKey) {
      const batchEnd = addDays(cursor, batchDays - 1);
      const effectiveEnd = batchEnd <= endDateKey ? batchEnd : endDateKey;
      const batch = await this.queryDateRange(
        collectionId,
        fieldPath,
        `${cursor} 00:00:00`,
  ...
  async queryCollection(collectionId, limit = QUERY_LIMIT) {
    return this.query({
      from: [{ collectionId }],
      limit,
    });
...
function normalizeArgs(args = {}) {
  const date = clean(args.date) || todayManila();
  const targetDate = clean(args.targetDate) || addDays(date, 1);
  const lookbackDays = Math.max(1, Number(args.lookbackDays || 0) || LOOKBACK_DAYS);
  const startDate = clean(args.startDate) || DEFAULT_START_DATE;
  const dryRun = Boolean(args.dryRun);
  const envFilePath = clean(args.envFilePath) || "";
  return { date, targetDate, lookbackDays, startDate, dryRun, envFilePath };
}

async function runAutoForwardPendingSchedules(args = {}) {
  const normalized = normalizeArgs(args);
  await loadEnvFile(normalized.envFilePath);

  const db = new FirestoreClient();
  const lookbackDate = addDays(normalized.date, -normalized.lookbackDays);
  const startDate = normalized.startDate && normalized.startDate > lookbackDate ? normalized.startDate : lookbackDate;
  const startDateKey = startDate;
  const endDateKey = normalized.date;

  const [scheduleRows, savedSourceRoutes, printedSourceRoutes, targetRoutes, closeRequestRows] = await Promise.all([
    db.queryDateRangeBatched("tbl_schedule", "task_datetime", startDateKey, endDateKey),
    db.queryDateRangeBatched(ROUTE_COLLECTION, "task_datetime", startDateKey, endDateKey),
    db.queryDateRangeBatched(PRINTED_ROUTE_COLLECTION, "task_datetime", startDateKey, endDateKey).catch(() => []),
    db.queryDateRange(ROUTE_COLLECTION, "task_datetime", `${normalized.targetDate} 00:00:00`, `${normalized.targetDate} 23:59:59`),
  ...
  const closedRouteScheduleIds = new Set(
    sourceRoutes
      .filter(isClosedOrCancelledRoute)
      .map(routeScheduleId)
      .filter(Boolean),
  );
  const approvedCloseRequestScheduleIds = new Set(
    closeRequestRows
      .filter(isApprovedCloseRequest)
      .map((row) => Number(row.schedule_id || 0))
      .filter(Boolean),
  );
  const blockedClosedScheduleIds = new Set([...closedRouteScheduleIds, ...approvedCloseRequestScheduleIds]);
  ...
  const openRows = scheduleRows.filter((row) => isOpenSchedule(row, normalized.date));
  ...
  for (const row of candidates) {
    ...
    const routeDocId = routeDocIdFor(scheduleId, normalized.targetDate);
    const routeNumericId = Number(
      `${normalized.targetDate.replace(/[^0-9]/g, "")}${String(forwarded.length + 1).padStart(5, "0")}`,
    );
    const targetDateTime = routeDateTimeFor(row, normalized.targetDate);
    ...
    const schedulePayload = {
      ...
      auto_carryover_target_date: normalized.targetDate,
    };
    ...
    if (!normalized.dryRun) {
      ...
      for (const oldRoute of oldRoutes) {
        await db.update(ROUTE_COLLECTION, oldRoute._docId || oldRoute.id, {
          ...
          remarks: `${clean(oldRoute.remarks || row.remarks || row.caller || "")} | Auto-carried over to ${normalized.targetDate}`,
          forwarded_to_date: normalized.targetDate,
    ...
    forwarded.push({
      scheduleId,
      staffId,
      from: dateOnly(row.task_datetime),
      to: normalized.targetDate,
      routeDocId,
    });
  }
...
module.exports = {
  runAutoForwardPendingSchedules,
  DEFAULT_START_DATE,
};
