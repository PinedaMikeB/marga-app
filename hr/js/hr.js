...
async function fetchHrPayrollSummary({ rebuild = false } = {}) {
    const response = await fetch(getMargabaseAdminUrl('/admin/hr-payroll-summary'), {
        method: rebuild ? 'POST' : 'GET',
        headers: rebuild ? { 'content-type': 'application/json' } : undefined,
        body: rebuild ? JSON.stringify({ rebuild: true }) : undefined,
        cache: 'no-store'
    });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok || payload?.error) {
        throw new Error(payload?.error?.message || `HR payroll summary HTTP ${response.status}`);
    }
    return payload;
}
...
async function loadHrModule() {
    ...
    try {
        const [summaryResult, positions, locations, fieldEvents, employeeDeductions] = await Promise.all([
            fetchHrPayrollSummary().catch(() => null),
        ...
        const summaryPayload = summaryResult?.payload && typeof summaryResult.payload === 'object' ? summaryResult.payload : null;
        const summaryEmployees = Array.isArray(summaryPayload?.employees) && summaryPayload.employees.every((row) => row && typeof row === 'object' && !Array.isArray(row))
            ? summaryPayload.employees.map((row) => ({ ...row }))
            : [];
        if (summaryEmployees.length) {
            HR_STATE.employees = summaryEmployees;
            HR_STATE.usingEmployeeSummary = true;
            HR_STATE.employeeSummaryBuiltAt = String(summaryResult?.builtAt || '');
            HR_STATE.fullEmployeeRosterLoaded = false;
        } else {
            const rebuiltSummary = await fetchHrPayrollSummary({ rebuild: true }).catch((error) => {
                console.warn('Unable to rebuild permanent HR/payroll summary; falling back to live employee roster.', error);
                return null;
            });
            const rebuiltPayload = rebuiltSummary?.payload && typeof rebuiltSummary.payload === 'object' ? rebuiltSummary.payload : null;
            const rebuiltEmployees = Array.isArray(rebuiltPayload?.employees) && rebuiltPayload.employees.every((row) => row && typeof row === 'object' && !Array.isArray(row))
                ? rebuiltPayload.employees.map((row) => ({ ...row }))
                : [];
            if (rebuiltEmployees.length) {
                HR_STATE.employees = rebuiltEmployees;
                HR_STATE.usingEmployeeSummary = true;
                HR_STATE.employeeSummaryBuiltAt = String(rebuiltSummary?.builtAt || '');
                HR_STATE.fullEmployeeRosterLoaded = false;
            } else {
        ...
        status.textContent = HR_STATE.usingEmployeeSummary && HR_STATE.employeeSummaryBuiltAt
            ? `${HR_STATE.employees.length.toLocaleString()} HR/payroll permanent summary rows loaded.`
...
function renderEmployees() {
    ...
    const statusNote = HR_STATE.usingEmployeeSummary
        ? `Shown from HR/payroll permanent summary${HR_STATE.employeeSummaryBuiltAt ? ` · built ${new Date(HR_STATE.employeeSummaryBuiltAt).toLocaleString('en-PH')}` : ''}.`
...
function renderPayrollModel() {
    ...
    document.getElementById('payrollStatus').textContent = `Payroll runs read the Employment & Payroll fields in tbl_employee plus attendance inside tbl_field_attendance. ${cutoff.title} detected. ${liveNotice} Government deductions show whenever the employee payroll source has a saved value. Active loan and cash-advance plans use PAYROLL DEDUCTIONS.xlsx amounts: 15th-cutoff plans on the 26th-10th run, 30th-cutoff plans on the 11th-25th run.`;
...
async function saveEmployeeDetails() {
    ...
    try {
        ...
        const refreshedSummary = await fetchHrPayrollSummary({ rebuild: true }).catch((error) => {
            console.warn('Unable to refresh HR/payroll permanent summary after employee save.', error);
            return null;
        });
        const refreshedPayload = refreshedSummary?.payload && typeof refreshedSummary.payload === 'object' ? refreshedSummary.payload : null;
        const refreshedEmployees = Array.isArray(refreshedPayload?.employees) && refreshedPayload.employees.every((row) => row && typeof row === 'object' && !Array.isArray(row))
            ? refreshedPayload.employees.map((row) => ({ ...row }))
            : [];
        if (refreshedEmployees.length) {
            HR_STATE.employees = refreshedEmployees;
            HR_STATE.usingEmployeeSummary = true;
            HR_STATE.employeeSummaryBuiltAt = String(refreshedSummary?.builtAt || '');
            HR_STATE.fullEmployeeRosterLoaded = false;
        } else if (mode === 'create') {
            HR_STATE.fullEmployeeRosterLoaded = true;
            HR_STATE.usingEmployeeSummary = false;
...
function normalizeEmployeeDeduction(record = {}) {
    ...
    const deductionPerPayroll15th = roundMoney(toNumber(firstPresent(record, ['deduction_per_payroll_15th', 'deductionPerPayroll15th'])));
    const deductionPerPayroll30th = roundMoney(toNumber(firstPresent(record, ['deduction_per_payroll_30th', 'deductionPerPayroll30th'])));
    ...
    return {
        ...
        balanceAmount,
        deductionPerPayroll,
        deductionPerPayroll15th,
        deductionPerPayroll30th,
...
function getEmployeeDeductionTotals(employee, period = getPayrollPeriod()) {
    ...
    getEmployeeDeductions(employee.id || employee._docId || '')
        ...
        .forEach((item) => {
            const amount = getDeductionAmountForPeriod(item, period);
...
function getPayrollDeductionRegister(period = getPayrollPeriod()) {
    return HR_STATE.employeeDeductions
        ...
        .map((item) => ({
            ...
            amount: getDeductionAmountForPeriod(item, period),
...
function getDeductionAmountForPeriod(item, period = getPayrollPeriod()) {
    const cutoff = getPayrollCutoffProfile(period);
    let perPayroll = toNumber(item.deductionPerPayroll);
    if (cutoff.key === 'first_cutoff' && toNumber(item.deductionPerPayroll15th) > 0) {
        perPayroll = toNumber(item.deductionPerPayroll15th);
    } else if (cutoff.key === 'second_cutoff' && toNumber(item.deductionPerPayroll30th) > 0) {
        perPayroll = toNumber(item.deductionPerPayroll30th);
    }
    return roundMoney(Math.max(0, Math.min(perPayroll, toNumber(item.balanceAmount))));
}
    ...
    tbody.innerHTML = rows.length
        ? rows.map((item) => {
            ...
            return `<tr>
                ...
                <td>${sanitize(item.deductionPerPayroll15th > 0 || item.deductionPerPayroll30th > 0
                    ? `${formatMoneyOrDash(item.deductionPerPayroll15th || 0)} / ${formatMoneyOrDash(item.deductionPerPayroll30th || 0)}`
                    : formatMoneyOrDash(item.deductionPerPayroll))}</td>
...
function fillCurrentGps(latitudeId, longitudeId, noticeId) {
    ...
    navigator.geolocation.getCurrentPosition(applyPosition, () => {
        navigator.geolocation.getCurrentPosition(applyPosition, () => {
            setNotice(notice, 'GPS permission was denied or the device could not return a location. Turn on location/precise location, then try again.', 'error');
        }, {
...
function validateTimeIn(location, employeePoint) {
    ...
    if (!hasCoordinates(employeePoint)) return { allowed: false, reason: 'Employee GPS point is missing. Tap Use current GPS first so the browser can fill the phone coordinates.' };
